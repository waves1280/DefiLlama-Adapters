...
async function getVaultContractsAddress(chain) {
  ...
  vaults.forEach((element) => {
    // v2 vaults
    if (element.isDeFi == false) {
      if (element.chain === chain)
        htAddress.push(element.share.address)
    }
  });
  return htAddress
}
...
chains.forEach(chain => {
  module.exports[chain] = {
    tvl: async (_, _b, _cb, { api, }) => {
      ...
      const cycleIndices = (await api.multiCall({ abi: abi.globalState, calls: vaults, })).map(i => i.cycleIndex)
      const bals = (await api.multiCall({ abi: abi.cycleState, calls: vaults.map((vault, i) => ({ target: vault, params: cycleIndices[i] - 1 })), })).map(i => i.fundValueAfterRequests)
